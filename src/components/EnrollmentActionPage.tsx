...
interface EnrollmentActionPageProps {
  mode: "student" | "teacher";
}

const EnrollmentActionPage = ({ mode }: EnrollmentActionPageProps) => {
 ...
 const fetchStudentOptions = async () => {
    try {
      type StudentsResponse = StudentBrief[] | PagedResponse<StudentBrief>;

      const response = await axiosClient.get<StudentsResponse>(
        "/students/filtered",
        {
          params: {
            pageNumber: 1,
  ...
  const fetchTeacherOptions = async () => {
    try {
      type TeachersResponse = TeacherBrief[] | PagedResponse<TeacherBrief>;
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        if (isStudentMode) {
          await Promise.all([fetchCircles(), fetchStudentOptions()]);
        } else {
          await Promise.all([fetchCircles(), fetchTeacherOptions()]);
        }
      } finally {
  ...
  return (
    <DashboardLayout
      title={isStudentMode ? "تسجيل طالب في حلقة" : "تسجيل أستاذ في حلقة"}
      subtitle={
        isStudentMode
          ? "اختر الحلقة والطالب لتسجيله فيها"
          : "اختر الحلقة والأستاذ لتسجيله فيها"
      }
    >
...
export default EnrollmentActionPage;